...
import { RefreshCw, Gift, Settings2, Ban, CheckCircle } from 'lucide-react';
import { UserSession, grantTokens, setTokenLimit, toggleSuspended } from '../../lib/adminApi';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-[#9a8a78]">{users.length} user{users.length !== 1 ? 's' : ''} total</p>
        <button
          onClick={onRefresh}
          disabled={loading}
      ...
      <div className="bg-white border border-[#e8ded0] rounded-xl overflow-hidden">
            ...
            <tbody className="divide-y divide-[#f0e8d8]">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-[#faf6ef] transition-colors">
                  ...
                  <td className="px-4 py-3 text-[#6b6b6b] capitalize text-xs">{user.subscription_status || 'free'}</td>
                  <td className="px-4 py-3">
        ...
        {users.length === 0 && !loading && (
          <div className="py-12 text-center text-[#9a8a78] text-sm">No users found</div>
        )}
        {loading && (
